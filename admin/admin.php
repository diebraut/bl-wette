...
{
?>
   <title>Nur für Administrator erlaubt!!!!</title>
<?php
}
?>
...
<?php
  if ($admin == "roland")
  {
	   ...
	   $db = mysqli_connect("$address", "$dbuser", "$dbpasswd");
	   ...
	   if(!isset($action))
	   {
	   	...
	   	<table>
	   		...
	   		<td><input type="button" value="Restore data for tblspieltag (status 29)" style="width:600px" onclick="document.location.href='<?php echo $thisfile?>?action=restoreTblSpieltag&admin=<?php echo $admin?>'" ></td>
	   		</tr>
	   	</table><br/><br/><br/>
	   	...
	   	<td><input type="button" value="Spieltag schließen" style="width:150px" onclick="document.location.href='<?php echo $thisfile?>?action=makeClosed'"></td>
	   	</tr><tr>
	   	...
	   	<tr>
	   	<td><input type="button" value="nächster Spieltag" style="width:150px" onclick="document.location.href='<?php echo $thisfile?>?action=makeOpen'"></td>
	   	</tr>
	   	<tr>
...
--   >
	   	...
	   	<tr>
	   	<td><input type="button" value="Mailtext ändern" style="width:150px" onclick="document.location.href='<?php echo $thisfile?>?action=changeMailText&admin=<?php echo $admin?>'"></td>
	   	</tr>
<!   -- Ende JayDee -->
	   	<tr>
	   	...
	   	<tr>
	   	<td><input type="button" value="Alles löschen (ausser die vereine)" style="width:150px" onclick="document.location.href='<?php echo $thisfile?>?action=reset&admin=<?php echo $admin?>'"></td>
	   	</tr>
	   	<tr>
	   	<td><fieldset>
	   ...
	   if($action == "import")
	   {
	   	...
	   	<table>
	   		<tr>
	   			<td>
	   			<b>Der import wurde ausgeführt!</b>
	   			</td>
	   		</tr>
	   ...
	   if($action == "sendRemember")
	   {
	   	...
	   	$result = mysqli_query( "select strEmail, strAlias from tblgamer where boolRemember=1");
	   	...
	   	?><br><br><input type="button" onclick="document.location.href='<?php echo $thisfile?>?admin=<?php echo $admin?>'" value="zum Hauptmenü"><?php
	   }
...
?>   
	   	...
	   	<br>&nbsp;<input type="button" onclick="document.location.href='<?php echo $thisfile?>?admin=<?php echo $admin?>'" value="zum Hauptmenü">
 <   ?
	   }
...
?>   
                    <br><br>gespeichert .....
	   	<br><br><input type="button" onclick="document.location.href='<?php echo $thisfile?>?admin=<?php echo $admin?>'" value="zum Hauptmenü">
 <   ?
...
#    Ende JayDee
	   ...
	   if($action=="setTeam")
	   {
	   	...
	   	{
	   		$result = mysqli_query( "select * from tblverein where lngIndex='$i'");
	   		echo mysqli_error();
	   	...
	   	?>
	   	<tr><td colspan="2"><input type="button" onclick="document.location.href='<?php echo $thisfile?>?admin=<?php echo $admin?>'" value="zum Hauptmenü"></td></tr>
	   	</table>
	   	<?php
	   }
	   ...
	   if($action=="saveTeam")
	   {
	   	...
	   	$strURL = $_POST['strURL'];
	   	$result = mysqli_query( "select * from tblverein where lngIndex=$id");
	   	if(mysqli_num_rows($result)!=0)
	   		mysqli_query($db,"UPDATE tblverein SET strName='$strName', strURL='$strURL' WHERE lngIndex=$id");
	   	else
	   		mysqli_query( "INSERT INTOtblverein(lngIndex,strName,strURL)Values($id,'$strName','$strURL')");
...
/*   		if(mysqli_num_rows($result)==9)
	   	...
	   	<?php
	   	echo "</tr>";
	   	echo "<tr><td colspan=\"3\"><input type=\"button\" value=\"zum Hauptmenü\" onclick=\"document.location.href='$thisfile'\"></td></tr>";
	   	echo "</table>";
	   ...
	   if($action == "makeClosed")
	   {
	   	...
	   	echo "<tr><td colspan=\"5\"><input type=\"button\" value=\"zum Hauptmenü\" onclick=\"document.location.href='$thisfile?admin=<?php echo $admin?>'\"></td></tr>";
	   	echo "</table>";
	   }
	   if($action=="setClosed")
	   {
	   ...
	   if($action=="reOpenSpieltag")
	   {
		 /*   
	   	 $newSeason = new bl_season();
                debug ("Start" . $newDay );
                $newSeason->setScores(); //from include_repair.php^
	   	 $id = $_GET['id'];
	   	 */
	     mysqli_query($db,"UPDATE tblspieltag SET intStatus=0 WHERE intTag=${newDay}");
	     mysqli_query($db,"UPDATE tblinfo SET dtmClose= AddTime(NOW(),'02:10')");		return;
	   }
	   if($action=="startNewSeason")
	   {
	   	$newSeason = new bl_season();
                echo ("Start xxxx" . $newDay );
                $newSeason->createTblSpieltageForSeason();
	   }

	   if($action=="restoreTblSpieltag")
	   {
			$newSeason = new bl_season();
            echo ("Start restore" . $newDay );
            $newSeason->restoreResults();
	   }
