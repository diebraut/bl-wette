...
function isWebsiteOnline($url) {
    ...
    // curl_setopt setzt im Folgenden mehrere Optionen f�r den cURL-Transfer
...
class bl_season {
    ...
    static $location = 'http://www.OpenLigaDB.de/Webservices/Sportsdata.asmx?WSDL';
    ...
    static $ArrIDVerein = array( 
                       array('54' ,'7'),  // Hertha BSC
                       array('123','2'),  // Hoffenheim
                       array('6'  ,'14'), // Bayer Leverkusen 
                       array('7'  ,'12'), // Borussia Dortmund
                       array('40' ,'1'),  // Bayern M�nchen
                       array('9'  ,'9'),  // Schalke 04
                       array('185','3'),  // D�sseldorf
                       array('1635' ,'15'), // RB Leibzig
                       array('79' ,'65'),  // N�rnberg
                       array('112' ,'8'),  // SC Freiburg
                       array('131','11'),  // VFL Wolfsburg 
                       array('134','17'), // Werder Bremen
                       array('91' ,'5'),  // Eintracht Frankfurt
                       array('95' ,'6'),  // FC Augsburg
                       array('81' ,'10'), // 1 FC Mainz 05
                       array('55' ,'16'), // Hannover 96
                       array('16','18'),  // VFB Stuttgart
                       array('87' ,'4'));  // Borussia M'gladbach
       ...
       {                            
          ...
          if (isWebsiteOnline(self::$location)) {            
              ...
              self::$isOpenDBAvailable = 0;   
                echo "Service (OpenLigaDB ist offline. Auswertung im Moment nicht m�glich ";
                RETURN;            
          }            
          self::$isOpenDBAvailable = 1;
    ...
    function setScores ( ) {
      ...
      if (self::$isOpenDBAvailable == 0 ) {
		   echo "Open DB not Available"; 
           RETURN;
      }
      $result = mysqli_query($db,"select * from tblspieltag;");
      ...
      //while($anzahl > 300)
      {
         ...
         {   
			 ...
			 echo ("update match(".$matchID.") goal1=".$goal1."--goal2=".$goal2."\n");
             mysqli_query($db, "UPDATE tblspieltag SET intGoal1 = $goal1, ".
											     "intGoal2 = $goal2 ".
                 	 	   "where intMatchIdFromOpenLigaDB=$matchID");
		 }								 
		 $anzahl--;
	  }	 
	  echo "done";
   ...
   /* erh�ht den Spieltag um 1. Die Anfangszeit des ersten  */
   ...
   function switchToNextSpieltag ( $currentDay ) 
   {
      ...
      {              
         ...
         $dtLastMatchDate  = "1900-01-01T00:00:00"; 
 	     ...
 	     foreach ($response->GetMatchdataByGroupLeagueSaisonResult as $matches){
	        foreach ($matches as $match)  {
                // f�r den auszuwertenden Spieltag nur die bereits 
                // terminierten spiele ber�cksichtigen
                if ( $match->matchIsFinished or ($currentGroupId >= $currentDay) )
                { 	
                    $d2 = date_create_from_format('Y-m-d*H:i:s', $match->matchDateTime);	
                    $d = date_format($d2,'Y-m-d H:i:s');	
         ...
         /* muss der aktuelle spieltag um eins erh�ht werden              */
         $nextTry = 0;
